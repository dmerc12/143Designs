...
class OrderMiddleware:
    ...
    def create_order(name: str, description: str, complete: bool, paid: bool) -> Order:
        ...
        if type(name) is not str:
            ...
            raise RuntimeError("The name field exceed 60 characters, please try again!")
        elif name == "":
            ...
            raise RuntimeError("The description field must be a sting, please try again!")
        elif len(description) > 255:
            logging.warning("Error in method create order, description too long")
            raise RuntimeError("The description field cannot exceed 255 characters, please try again!")
        elif description == "":
            logging.warning("Error in method create order, description left empty")
            raise RuntimeError("The description field cannot be left empty, please try again!")
        elif type(complete) is not bool:
            logging.warning("Error in method create order, complete not a boolean")
            raise RuntimeError("The complete field must be a boolean, please try again!")
    ...
    def get_order(order_id: int) -> Order:
        ...
        if type(order_id) is not int:
            ...
            raise RuntimeError("The order ID field must be an integer, please try again!")
        else:
            order = Order.objects.get(pk=order_id)
            if not order:
                logging.warning("Error in method get order, no order found")
                raise RuntimeError("No order found, please try again!")
            else:
    ...
    def get_all_orders() -> List[Order]:
        ...
        logging.info("Finishing method get all objects with orders: " + str([order.__str__() for order in orders]))
        return orders
