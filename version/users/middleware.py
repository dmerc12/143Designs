...
class UserMiddleware:
    ...
    def create_user(request, form):
        ...
        logging.info('Beginning user middleware method create user with username: ' + str(username) + ', email: ' + str(email) + ', password: ' + str(password) + ', and confirmation password: ' + str(confirmation_password))
    ...
    def update_user(request, form):
        ...
        password = form.cleaned_data.get('password')
        confirmation_password = form.cleaned_data.get('confirmation_password')
        logging.info('Beginning user middleware method update user with username: ' + str(username) + ', email: ' + str(email) + ', password: ' + str(password) + ', and confirmation password: ' + str(confirmation_password))
        if type(username) is not str:
            ...
            logging.warning('Error in user middleware method update user, password not a string')
            raise RuntimeError('The password field must be a string, please try again!')
        elif len(password) > 60:
            logging.warning('Error in user middleware method update user, password too long')
            raise RuntimeError('The password field cannot exceed 60 characters, please try again!')
        elif password == '':
            logging.warning('Error in user middleware method update user, password left empty')
            raise RuntimeError('The password field cannot be left empty, please try again!')
        elif type(confirmation_password) is not str:
            logging.warning('Error in user middleware method update user, confirmation password not a string')
            raise RuntimeError('The confirmation password must be a string, please try again!')
        elif len(confirmation_password) > 60:
            logging.warning('Error in user middleware method update user, confirmation password too long')
            raise RuntimeError('The confirmation password cannot exceed 60 characters, please try again!')
        elif confirmation_password == '':
            logging.warning('Error in user middleware method update user, confirmation password left empty')
            raise RuntimeError('The confirmation password cannot be left empty, please try again!')
        else:
